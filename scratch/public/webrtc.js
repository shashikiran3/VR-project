...
function prepareCall() {
  ...
  // once remote stream arrives, show it in the remote video element
    ...
    peerConn.addStream    = onAddStreamHandler;
  ...
  peerConn.createOffer(
    function (offer) {
      ...
      peerConn.setLocalDescription(new RTCSessionDescription(off), 
        function() {
          wsc.send(JSON.stringify({"sdp": off,type:"video-offer"}));
        }, 
        function(error) { console.log(error);}
      );
    }, 
...
function createAndSendAnswer() {
  console.log("creating and sending an answer");
...
function onAddStreamHandler(evt) {
  console.log("inside onAddStreamHanlder");
  videoCallButton.setAttribute("disabled", true);
  endCallButton.removeAttribute("disabled"); 
  // set remote video stream as source for remote video HTML5 element
  remoteVideo.src = URL.createObjectURL(evt.stream);

 // document.getElementById("remoteVideo").src = evt.stream;
};
